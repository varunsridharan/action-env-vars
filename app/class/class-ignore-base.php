...
class Ignore_Base {
	...
	public function run( $type ) {
		...
		if ( ! empty( $files_to_check ) && is_array( $files_to_check ) ) {
			foreach ( $files_to_check as $file ) {
				_( 'Checking In : ' . $this->workspace() . '/' . $file );
	...
	protected function handle_file_exists( $path ) {
		...
		if ( 'ignore' === $this->type ) {
			_( '✔️ DISTIGNORE File Found : ' . $ignore_file );
			_( 'Added Predefined DISTIGNORE' );
		} else {
			_( '✔️ Assets DISTIGNORE File Found : ' . $ignore_file );
			_( 'Added Predefined Assets DISTIGNORE' );
		}
	...
	protected function handle_file_not_exits( $path ) {
		if ( 'ignore' === $this->type ) {
			_( '⚠️ DISTIGNORE File Not Found ! | Creating Default ' );
		} else {
			_( '⚠️ Assets DISTIGNORE File Not Found ! | Creating Default ' );
		}
